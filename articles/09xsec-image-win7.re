...
前章の構成に沿っていますが、違うのはWindows上でPowerPointを使っているところです。
OSやソフトが変わるとできることも変わってきますので、内容が簡略化され、Windowsに特化した情報が加筆されています。（Text：暗黙の型宣言）
...
一つ目の方法は画像ごとの適切な大きさを見つけるのが試行錯誤となり、手間がかかります。
