...
macOSでKeynote.appを使って図を作成し、原稿に埋め込む方法を紹介します。また印刷用に高解像度の画像を作成する方法も説明します。「Keynote.app」とは、macOSやiOSにおいて無料で使えるプレゼンテーションソフトです。基本的な作図機能が備わっているので、以降ではKeynote.appを使った作図方法を紹介します。

なおこの章はmacOSのみを対象としています。ご了承ください。（Text：カウプラン）
...
　１．画面左上隅にあるリンゴのアイコンをクリックし、「App Store...」を選びます。

　２．App Storeアプリが起動したら、右上の検索欄で「Keynote」と入力しReturnを押します。

　３．検索結果のいちばん最初に「Keynote」が出てくるので、「入手」をクリックします。
...
　１．Keynote.appを起動し、テーマ「ホワイト」を選択してスライドを新規作成します。

　２．ウィンドウ右上の「フォーマット」タブ→「マスターを変更」ボタンを押し、「空白」を選びます。これで白紙の状態になりました。

　３．試しに、何か適当な図を描いてみましょう。ウィンドウ上部にある「図形」アイコンをクリックし、適当な図形を選んで、位置やサイズを変更します。ここでは@<img>{keynote-screenshot}のような図を描いてみました。

　４．Command-Sを押して、スライドを保存します。保存先はプロジェクトと同じフォルダ、ファイル名は「images.key」がいいでしょう@<fn>{image-folder}。
...
　１．Keynote.appの画面で、Command+Aを押して図形をすべて選択し、Command+Cでクリップボードにコピーします。

　２．Preview.appを起動し、Command+Nを押すと、先ほどコピーした内容を使って画像が作成されます（@<img>{keynote-newimage}）。

　３．そのままCommand+Sを押して、画像を保存します。保存先は「images」@<fn>{images}、フォーマットは「PNG」、ファイル名はたとえば「example1.png」にします。
...
　１．ウィンドウ上部の「図形」アイコンから四角形を選びます。

　２．四角形を、図形を覆うくらいの位置と大きさに変更したあと、Commad+Shift+Bを押して最背面に移動させます。

　３．ウィンドウ右上の「フォーマット」アイコン→「スタイル」タブを選びます。「塗りつぶし：塗りつぶしなし、枠線：枠線なし、シャドウ：シャドウなし」に変更します。

　４．ウィンドウ右上の「フォーマット」アイコン→「配置」タブを選びます。サイズを、たとえば「幅：900pt、高さ：300pt」に変更します（@<img>{keynote-bgbox}）。幅と高さは、紙面の幅と図形の高さに合わせて調整してください。
...
　１．Keynote.appでスライドを開き、ウィンドウ右上の「書類」アイコン→「書類」タブを選び、下のほうにある「スライドのサイズ」で「カスタムのスライドサイズ...」を選びます。

　２．デフォルトでは「幅：1024pt、高さ：768pt」になっているので、それぞれ5倍して「幅：5120pt、高さ：3840pt」にして、「OK」ボタンを押します（@<img>{keynote-enlarge}）。
