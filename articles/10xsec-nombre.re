...
ノンブルとは、ページの1枚目から順番につけれらた番号のことです。ページ番号と似ていますが、読者のためではなく、印刷所がページの順番を確認するためにつけられます。ページ順のトラブルを避けるためにたいていの印刷所ではノンブルが必須であり、たとえページ番号があってもノンブルがないと印刷を受けつけてくれません。

この章では、ノンブルとページ番号の違いを詳しく説明したあとに、PDFにノンブルを入れる方法を紹介します。（Text：カウプラン）
...
なおノンブルもページ番号も、表紙1と表4にはつけません。表紙1と表4は、本文とは用紙も印刷工程も違い、ページ順番の確認対象ではないためノンブルは必要ありません。
...
　１．Adobe Acrobat Reader DC.appをインストールしていない場合は、「Acrobat Reader DC」でインターネット検索して、インストールしてください。

　２．Finder.appで、Controlキーを押しながら対象のPDFファイルをクリックし、「このアプリケーションで開く」→「Adobe Acrobat Reader DC.app」を選びます。

　３．Adobe Acrobat Reader DC.appが起動するので、Command+Dを押すか、メニューから「ファイル」→「プロパティ...」を選びます。

　４．「文書のプロパティ」ダイアログが出るので、「フォント」タブを選びます（@<img>{acrobat-reader}）。フォント名の後ろに「埋め込みサブセット」とついているフォントは、PDFファイルに埋め込まれています。
...
　１．対象のPDFファイルをPreview.appで開きます。

　２．Preview.appで、メニューから「ファイル」→「PDFとして書き出す...」を選びます。

　３．ファイル名を指定して、新しいPDFファイルとして保存します。
